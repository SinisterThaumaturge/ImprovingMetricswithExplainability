...
def out_MT_seg_level_correlation_wmt17(lp_set, eval_metric, f):
  ...
  for l in lines:
    ...
    if system not in manual[lp]:
      manual[lp][system] = {}
    if sid not in manual[lp][system]:
        ...
        if (l.find("hybrid")==-1) and (l.find("himl")==-1):
          ...
          if len(c) < 6:
            ...
            if metric not in metrics[lp]:
              metrics[lp][metric] = {}
            if system not in metrics[lp][metric]:
              metrics[lp][metric][system] = {}
            if sid not in metrics[lp][metric][system]:
              metrics[lp][metric][system][sid] = score
    res_str = ""
    for lp in manual:
      if lp not in lp_set: continue
      if lp not in metrics:
      ...
      for metric in metrics[lp]:
        allthere = True
        for system in manual[lp]:    
            if not system in metrics[lp][metric]:
                systems = system.split("+")
                for sid in manual[lp][system]:
                  score = 0
                  for s in systems:
                    score += metrics[lp][metric][s][sid]
                metrics_scores.append(score/len(systems))
                ref_scores.append(float(manual[lp][system][sid]))
            else:  
              for sid in manual[lp][system]:
                metrics_scores.append(float(metrics[lp][metric][system][sid]))
                ref_scores.append(float(manual[lp][system][sid]))
        res_str = res_str + metric + '\t'+ lp +"\t" + pearson_and_spearman(metrics_scores, ref_scores) +"\n"
  return pd.read_csv(StringIO(res_str), sep="\t", header=None)
            

def output_MT_correlation(lp_set,f):    
    lines = [line.rstrip('\n') for line in open(f)]
    lines.pop(0)
    manual = {}
    for l in lines:
    ...
    for lp in manual:
      ...
      results = pd.read_csv(StringIO(s), sep=" ")
      for key in results:
        if key == "HUMAN" or key == "SYSTEM" or key == "LP":
          continue
        print(key+"\t"+lp +"\t" + pearson_and_spearman(results['HUMAN'], results[key]))
...
import shutil   
def print_seg_level_correlation_wmt17(metric, data, lp_set, f = "DA-seglevel.csv"):
    results = pd.concat(data, ignore_index=True)
    ...
    #outputs = out_MT_seg_level_correlation_wmt17(lp_set,  [metric + '.seg.score.gz'], f)
